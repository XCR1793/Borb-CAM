...
if (!sliceSegments.empty()) {
    int groupCount = (int)sliceSegments.size();

    for (int i = 0; i < groupCount; ++i) {
        const Lines& slice = sliceSegments[i];

        // Assign a unique color per discontinuous group
        float hue = fmodf((360.0f * i) / (float)groupCount, 360.0f);
        ...
        // Optional debugging
        // DrawCube(slice.startPosition, 0.02f, 0.02f, 0.02f, GREEN);
        // DrawCube(slice.endPosition,   0.02f, 0.02f, 0.02f, RED);
    }
}
