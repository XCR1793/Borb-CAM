...
if (runSlice) {
    ...
    for (size_t i = 0; i < orderedSlices.size(); ++i) {
        std::vector<Line>& slice = orderedSlices[i];
        Point& entry = rayEntries[i];
        Point& exit = rayExits[i];

        if (i > 0) {
            Point& prevExit = rayExits[i - 1];
...
//         // Optional flip based on previous slice exit
//         if (!orderedSlices.empty()) {
//             Point prevExit = orderedSlices.back().back().endLinePoint;
//             float distToStart = Vector3Distance(prevExit.Position, hitEntry);
//             float distToEnd   = Vector3Distance(prevExit.Position, hitExit);

//             if (distToEnd < distToStart) {
//                 std::reverse(slice.begin(), slice.end());
//                 for (Line& l : slice) std::swap(l.startLinePoint, l.endLinePoint);
//                 std::swap(hitEntry, hitExit);
//                 std::swap(entry.Normal, exit.Normal);
//                 std::swap(entry, exit);
//             }

//             // Connect from last slice exit to this slice entry (AABB hit)
//             combinedPath.push_back(Line{
//                 .startLinePoint = prevExit,
//                 .endLinePoint = { hitEntry, entry.Normal },
//                 .type = 2
//             });
//         }
...
//         // AABB entry ray
//         combinedPath.push_back(Line{
//             .startLinePoint = { hitEntry, entry.Normal },
//             .endLinePoint = entry,
//             .type = 2
//         });
...
//         combinedPath.push_back(Line{
//             .startLinePoint = exit,
//             .endLinePoint = { hitExit, exit.Normal },
//             .type = 2
//         });
